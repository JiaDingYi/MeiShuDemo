...
@interface MSMTACustomFeedAdMeta ()
@property(nonatomic,strong) UIView<MentaNativeAdViewProtocol> *relatedView;

@end

@implementation MSMTACustomFeedAdMeta
...
-(void)attachAd:(UIView *)container
    ...
    if (!self.relatedView) {
        self.relatedView = self.thirdPlatformNativeData.nativeAdView;
        self.relatedView.frame = container.bounds;
        [container insertSubview:self.relatedView atIndex:0];
    }
    ...
    [self.thirdPlatformNativeData registerClickableViews:clickViews closeableViews:@[]];
}
//解绑数据
- (void)unAttachAd{
...
- (void)attachAd:(UIView * _Nonnull)container renderViews:(NSArray<UIView *> * _Nonnull)renderSubViews clickView:(NSArray<UIView *> * _Nonnull)clickViews closeView:(UIView * _Nullable)closeView presentVc:(UIViewController * _Nonnull)presentVc {
    ...
    [self.logo removeFromSuperview];
    [container addSubview:self.logo];
    self.logo.frame = CGRectMake(container.frame.size.width - 32, container.frame.size.height - 16, 16, 16);
    [self.thirdPlatformNativeData registerClickableViews:clickViews closeableViews:@[]];
}
